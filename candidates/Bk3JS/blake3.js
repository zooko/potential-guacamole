...
const { compress4x, wasmMemoryBuffer } = await (async function () {
    ...
    function writeLebU32(value, pos) {
        for (let i = 0; i < 5; ++i) {
    ...
    let _next_m = 0;
    function gi(a, b, c, d, d_rot, b_rot) {
    ...
    return {
        ...
        compress4x: wasmInstance.exports.compress4x
    };
})();
...
function reverseByteorderOnBigEndian(n) {
    n |= 0;
    if (IsBigEndian) {
        return (
            ((n & 0xff) << 24) |
...
export function hash(input) {
    ...
    for (let wordOffset = 0; wordOffset < fullFourBlocksWords;) {
        ...
        for (let innerBlock = 0; innerBlock < 16; ++innerBlock) {
            ...
            if (!IsBigEndian) {
                for (let i = 0; i < 64; i += 4, ++wordOffset) {
                    wasmMemoryU32[i] = inputWords[wordOffset];
                    wasmMemoryU32[i + 1] = inputWords[wordOffset + 256];
                    wasmMemoryU32[i + 2] = inputWords[wordOffset + 512];
                    wasmMemoryU32[i + 3] = inputWords[wordOffset + 768];
                }
            } else {
                for (let i = 0; i < 64; i += 4, ++wordOffset) {
            ...
            if (!IsBigEndian) {
                for (let i = 0; i < 4; ++i) {
                    ...
                    wasmMemoryU32[112 + i] = (chunkCounter + i) | 0;
                    wasmMemoryU32[116 + i] = ((chunkCounter + i) / 0x100000000) | 0;
                    wasmMemoryU32[120 + i] = BLOCK_LEN;
                    wasmMemoryU32[124 + i] = flags |
                        (innerBlock === 0
                            ? CHUNK_START
                            : innerBlock === 15
                                ? CHUNK_END
        ...
        if (!IsBigEndian) {
            for (let c = 0; c < 4; ++c) {
                for (let i = 0; i < 8; ++i) {
                    cvStack[cvStackPos + i] = wasmMemoryU32[(16 + i) * 4 + c];
                }
                cvStackPos += 8;
            }
        } else {
    ...
    if (cvStackPos == 0) {
        compress(
            ...
            0,
            length - offset,
            flags | ROOT | CHUNK_END | (numRemainingFullBlocks === 0 ? CHUNK_START : 0)
        );
    } else {
        compress(
            cvStack,
